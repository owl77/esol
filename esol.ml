...
type signature = I | T of int;;

type argumentSig =   Signature of signature list  | IndividualSig;;
...
let rec siglist_arity siglist = match siglist with 
 I::b -> if (siglist_arity b > -1) then (1 + siglist_arity b) else -1
|(T n)::b -> -1
|_ -> 0;;

let sig_check siglist = if siglist_arity siglist = -1 then false else true;;
...
let const_dic = ref [("*", IndividualSig); ("C", Signature [I]) ];;
...
let term_arity term = match term with
  Lambda (x,g) -> List.length x
 | Constant (s, Signature n) -> siglist_arity n 
 | Variable (s, (Signature n,i)) -> siglist_arity n
 | Constant (s, IndividualSig)  -> -2
 | Variable (s, (IndividualSig,i)) -> -2;;

 let rec sig_match siglist list2 = match siglist, list2 with
   [], [] -> true
  | I::b, c::d -> if term_arity c = -2 then sig_match b d else false
  | (T n)::b , c::d   -> if term_arity c = n then sig_match b d else false 
  |_ -> false ;;

  let rec all_indiv termlist = match termlist with
    None -> false
   |Some (a::b) -> if term_arity a = -2 then all_indiv (Some b) else false
   |_ -> true;;



let check_app t l = match t,l with 
                 Some Lambda (x,g), Some m  -> ((List.length x) = (List.length m))  && (indVarCheck x) && all_indiv l
                |Some Constant (s,Signature n), Some m -> (List.length m) = (List.length n)  && sig_match n m 
                |Some Variable (s,(Signature arg,i)), Some m -> (List.length m) = (List.length arg) && sig_check arg 
                |_ -> false;; 
